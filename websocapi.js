...
function callWebSocAPI(
  ...
  request.post(postData, (err, res, body) => {
    if (!err && res.statusCode === 200) {
      var item = parse(body);
      if (!moreC) callback(item);
      else {
        total.push({
          term: term,
          section: item[0].departments[0].courses[0].sections[0],
          courseName: item[0].departments[0].courses[0].name,
          termName: item[0].courseTerm
        });
        callback();
      }
    } else {
...
function parse(XMLBody) {
  ...
  root.find("school").each(function() {
    const school = $(this);
    const schoolObj = new classes.School(
      school.attr("school_name"),
      $("school_comment", school).text()
    );
    schools.push(schoolObj);

    school.find("department").each(function() {
      const dept = $(this);
      const deptObj = new classes.Department([
        dept.attr("dept_code"),
        dept.attr("dept_name")
      ]);
      ...
      dept.find("course").each(function() {
        const course = $(this);
        const courseObj = new classes.Course(
          [
            dept.attr("dept_code"),
            course.attr("course_number"),
            course.attr("course_title")
          ],
          $("course_prereq_link", course).text(),
          $("course_comment", course).text()
        );
        ...
        course.find("section").each(function() {
          ...
          const sectionData = {
            ...
            instructors: $("sec_instructors > instructor", section)
              .map(function() {
                return $(this).text();
              })
              .get(),
            finalExam: $("sec_final", section)
              .text()
              .trim(),
            maxCapacity: $("sec_enrollment > sec_max_enroll", section).text(),
            numCurrentlyEnrolled: [
              $("sec_enrollment > sec_enrolled", section).text(),
              $("sec_enrollment > sec_xlist_subenrolled", section).text() === ""
                ? $("sec_enrollment > sec_enrolled", section).text()
                : $("sec_enrollment > sec_xlist_subenrolled", section).text()
            ],
            numOnWaitlist:
              $("sec_enrollment > sec_waitlist", section).text() === ""
                ? "0"
                : $("sec_enrollment > sec_waitlist", section).text(),
            numRequested: $(
              "sec_enrollment > sec_enroll_requests",
              section
            ).text(),
            numNewOnlyReserved:
              $("sec_enrollment > sec_new_only_reserved", section).text() ===
              $("course_code", section).text()
                ? "0"
                : $("sec_enrollment > sec_new_only_reserved", section).text(),
          ...
          $("sec_meetings > sec_meet", section).each(function() {
            meetings.push([
              $("sec_days", this).text() + $("sec_time", this).text(),
              $("sec_bldg", this).text() + " " + $("sec_room", this).text()
            ]);
          });
...
function a(items, callback) {
  ...
  for (var i = 0; i < length; ++i) {
    obA.push({
      courseCodes: items["courseCodes" + i],
      term: items["term" + i]
    });
  }

  async.each(obA, callWebSocAPI, function(error) {
    if (error) {
      return;
    } else {
...
function callWebSocAPI1(items, callback) {
  moreC = false;
  callWebSocAPI(items, re => {
    callback(re);
  });
}
module.exports.a = a;
module.exports.callWebSocAPI1 = callWebSocAPI1;
